...
/*
 * shows appointments by instructor, all appointments both taken andavailable are shown
...
<a href="<?php echo $url;?>/webapps/octt-octetsign-bb_bb60/links/welcome.jsp?course_id=<?php echo $_POST["course_id"]; ?>">APPOINTMENTS</a> > VIEW</span><br>
<br>
...
while($ap = mysql_fetch_array($res)){
	...
	if($today < strtotime($ap[0]." ".$ap[1])){ //if appointment is pending
		$signup = false; //student cannot sign up for another appointment
	}
}
	...
	if($today < strtotime($appts[0]." ".$appts[1])){
		...
		if(strcmp($date, $appts[0])!=0){ //display new date header if it's different from the previous one
			...
			<table width="900"  border="0">
			  <tr class="style6">
				<td width="60" align="middle">Time</td>
				<td width="120" align="middle">Duration (minutes) </td>
				<td width="175" align="middle">Appointment with </td>
				<td></td>
			  </tr>
			<?php
			$closetable = true;	
		...
		<td height="30" align="center"><?php echo $appts[3]; ?></td>
		<td height="30" align="center"><?php echo stripslashes($professor['name']); ?></td>
		<td height="30"><?php if($appts[2]=='t'){ //appointment status is set to "taken"
			...
			echo "Taken by: ".stripslashes($student['name']); //display the results
			...
			{ ?>
				...
				Reason for cancel <input name="reason" type="text" size="30">
...
?>
 ...
 <br>
 &nbsp;&nbsp;This intructor does not have any available appointments in the database.<br>
 </span> <?php
}
?>
<table width="900"  border="0" class="style6"><tr><td>
