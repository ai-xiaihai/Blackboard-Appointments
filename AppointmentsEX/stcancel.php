...
<body>
	<span class="style1"><br>&nbsp;&nbsp;<?php include("bread.php"); ?> > Student Cancel</span><br>
	<span class="style6">
		<?php 
	//code here
	...
	//find if anyone signed up for the appointment
	  ...
	  if($student = mysql_fetch_assoc($result)){
		...
		if($appttime > $endtime){ //if appointment is within 1 days
			...
			if( $prof = mysql_fetch_assoc(mysql_query($sql, $connection)) ){
				...
				if( $professor = mysql_fetch_assoc(mysql_query($sql, $connection)) ){ //$professor contains instructor's information			
					...
					echo "Processing a student request to cancel appointment...\n"; 
					...
					}else{echo "Could not send email to the instructor notifying them of the change.<br> 
		...
		echo "<b> FAILED </b>Cancellation of appointments within 24 hours is not permitted. The instructor will receive NO email regarding the change. CONTACT THE PROFESSOR DIRECTLY VIA EMAIL.<br>";
			//cancel the appointment
